...
import 'package:braketpay/screen/merchantcreateloan.dart';
...
class MarketPlaceLoan extends StatefulWidget {
  MarketPlaceLoan({Key? key}) : super(key: key);

  @override
  State<MarketPlaceLoan> createState() => MarketPlaceLoanState();
}

class MarketPlaceLoanState extends State<MarketPlaceLoan> {
     ...
     height: kToolbarHeight + MediaQuery.of(context).padding.top,
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Text('Loans', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.white, fontSize: 22),),
            ],
          ),
          Row(
            children: [
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (snapshot.hasData) {
                            ...
                            if (currentCategory == 'All') {
                              sortedProducts = snapshot.data!['LOAN'];
                            } else {
                              snapshot.data!['LOAN'].forEach((element) {
                                print(element);
                                if (element['Payload']['loan_categories']?['category_1'] == currentCategory) {
                          ...
                          return Expanded(
                            child: RefreshIndicator(
                              key: brakey.refreshLoanMarket.value,
                              onRefresh: () async {
                                  // brakey.reloadUser('');
                                  final product = await fetchMarketContracts(
                                      brakey.user.value?.payload?.publicKey ?? '',
                                              ...
                                              child: Center(
                                                child: Column(
                                                  ...
                                                  children: [
                                                    ...
                                                    SizedBox(height:20),
                                                    Flexible(child: Text(hasLoadError ? loadErrorMsg : "Fetching ${toTitleCase(_templates[index]['Payload']['loan_title'])}", textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey, fontSize: 23),))
                                                  ],
                                                ),
                                              ),
                                            );
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            Padding(
                                              ...
                                              child: Text(toTitleCase(_templates[index]['Payload']['loan_title']??''),
                                                  maxLines: 1,
                                                  textAlign:
                                                      TextAlign
                              ...
                              child: Center(
                                    ...
                                    children: [
                                      ...
                                      RoundButton(
                                          ...
                                          color2: Colors.black,
                                          onTap: () {
                                            brakey.refreshLoanMarket.value?.currentState?.show();
                                          })
                                    ],
                                  )),
                            );
